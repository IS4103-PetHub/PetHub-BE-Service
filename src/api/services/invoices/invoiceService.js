...
/*
  TODO: 
    * Change to class
    * Endpoint to generate invoice (or just leave as function for order service to call)
    * Modify function to save to S3 
*/
...
function generate(data, path) {
  let doc = new PDFDocument({ size: "A4", margin: PAGE_MARGIN, bufferPages: true });
  ...
  // main content
  yEndPosition = generateItems(doc, data);
...
function generatePethubImage(doc) {
  doc.image("pethublogo.png", 410, 45, { scale: 0.1 });
  doc
    .fillColor("#606060")
    .fontSize(12)
...
generate(InvoiceData, "IN04102023-1.pdf");
